...
| **M15** | [MB](../next/MB.md) | Anti-toxic lifecycle + internal PnL feedback (v2.1.1 nouveau facteur, ranking-based active, gate arbitrage_bot, probation 0.25×, auto-blacklist) | ✅ shipped | 8 commits MB.x + 1 follow-up MB.9 ship 2026-05-02 (v2.1.1 parallel shadow compute). Migration Alembic 0009 (MB.9 zéro migration). Bloque MF jusqu'à 30j de collecte `internal_pnl_score`. |
| **M15 MB.9** | (follow-up) | v2.1.1 parallel shadow compute (combler le gap M15 — `compute_score_v2_1_1` désormais invoqué en parallèle de v2.1 pilote sous flag `SCORING_V2_1_1_SHADOW_DAYS>0`, débloque cutover J+30) | ✅ shipped 2026-05-02 | 1 commit. +1 setting + cross-field validator + helper paramétré `_is_shadow_active(version)` + 5 unit tests. Aucune migration Alembic (head reste 0010). Aucune touche `discovery/scoring/` (versioning sacré). Default off : zéro régression pour les users ne préparant pas un cutover. Alimente `/traders/scoring` (M21) en mode dual-version. |
