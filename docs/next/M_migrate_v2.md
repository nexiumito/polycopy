# M_migrate_v2 — Polymarket CLOB V2 + pUSD migration
...
### MZ.1 — Bumper SDK `py-clob-client` → `py-clob-client-v2`
...
### MZ.2 — Adapter Order struct V2 + EIP-712 signing path
...
### MZ.3 — Migrer FeeRateClient M16 vers `getClobMarketInfo()`
...
### MZ.4 — Polymarket USD collateral (env vars + helper wrap)
...
### MZ.5 — Builder code support (optionnel mais ROI direct)

- **Location** : [src/polycopy/config.py](../../src/polycopy/config.py) +
  fichier executor V2 (post-MZ.1).
- **Ce qu'il faut faire** :
...
- **Charge item** : 0.5 jour
- **Décision** : à shipper **avec ou après MZ.1-MZ.3** selon bandwidth.
...
### MZ.6 — Update tests intégration vs `clob-v2.polymarket.com`
...
### MZ.7 — Procédure cutover ops + rollback
...
- **H-V2-2** : la doc `/resources/contracts` liste-t-elle les adresses
  ...
  pendant la spec writing. Si absence → bloquer MZ.4 (spec ne peut pas
...
|---|---|---|
| MZ.1 | Doc V2 §SDK Migration | py-clob-client-v2 GitHub README |
| MZ.2 | Doc V2 §Order signing flow + EIP-712 | EIP-712 spec ([eips.ethereum.org/EIPS/eip-712](https://eips.ethereum.org/EIPS/eip-712)) |
| MZ.3 | Doc V2 §getClobMarketInfo | M16 spec [docs/specs/M16-dynamic-fees-ev.md](../specs/M16-dynamic-fees-ev.md) §11 (rétrocompat path) |
| MZ.4 | Doc V2 §Polymarket USD + Collateral Onramp | docs.polymarket.com/resources/contracts |
| MZ.5 | Doc V2 §Builder Program | polymarket.com/settings?tab=builder UI |
| MZ.6 | Test environnement V2 | tests/integration/ pattern M2/M11 |
| MZ.7 | docs/todo.md §14 | Discord Polymarket Dev hands-on onboarding |
...
Lis [docs/next/M_migrate_v2.md](docs/next/M_migrate_v2.md) en entier.
...
[docs.polymarket.com/v2-migration](https://docs.polymarket.com/v2-migration)
+ les 7 items MZ.1 → MZ.7 de scope.
...
- **Ordre commits recommandé** : MZ.1 (SDK bump) → MZ.2 (Order struct +
  signature) → MZ.3 (FeeRateClient swap) → MZ.4 (env vars + wrap helper) →
  MZ.5 (builder code optionnel) → MZ.6 (tests intégration) → MZ.7 (procédure
...
- `py-clob-client-v2` n'est pas encore publié sur PyPI au moment de la
  spec writing → impossible de rédiger MZ.1 avec confiance.
- Les adresses pUSD ou CollateralOnramp ne sont pas publiées sur
  `/resources/contracts` au moment de la spec writing → MZ.4 inactionnable.
...
- **Q-V2-2** : Builder code fee rebate % exact non documenté — solliciter
  ...
  builder côté polycopy. Si rebate < 10 bps → ROI faible, MZ.5 reportable.
