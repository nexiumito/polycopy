...
> **Status M12** ✅ — scoring v2 (formule hybride + gates durs + shadow period). Sous-package isolé `src/polycopy/discovery/scoring/v2/` : 6 facteurs purs (Sortino+Calmar `risk_adjusted`, Brier-skill `calibration`, pair→wallet `timing_alpha` neutre v1, HHI catégories Gamma `specialization`, fraction mois PnL>0 `consistency`, (1-zombie)×sizing_stability `discipline`), winsorisation p5-p95 pool-wide + normalisation 0-1, pondération figée `0.25/0.20/0.20/0.15/0.10/0.10`. 6 gates durs pré-scoring fail-fast : `cash_pnl_90d>0`, `trade_count_90d≥50`, `days_active≥30`, `zombie_ratio<0.40`, `not blacklisted`, `not in WASH_CLUSTER_WALLETS` — wallet rejeté écrit `trader_events.event_type="gate_rejected"`, **jamais scoré**. `MetricsCollectorV2` compose `MetricsCollector` M5 (zéro ligne modifiée) + nouveau `TraderDailyPnlWriter` (scheduler 24h) qui peuple la table `trader_daily_pnl` (migration 0006 additive) source de l'equity curve. Shadow period dual-compute : `SCORING_VERSION=v1` + `SCORING_V2_SHADOW_DAYS>0` → v1 pilote `DecisionEngine`, v2 écrit `trader_scores` en parallèle (audit). Cutover manuel conditionné à rapport `scripts/backtest_scoring_v2.py` montrant `brier_top10_v2 < brier_top10_v1 - 0.01` sur set labelé `assets/scoring_v2_labels.csv`. Dashboard `/traders/scoring` rend le tableau v1|v2|delta_rank + agrégats (Spearman, top-10 delta, shadow status). Versioning sacré : chaque row `trader_scores` porte sa `scoring_version` append-only, pas de rewrite rétroactif. Voir `specs/M12-scoring-v2.md`.
